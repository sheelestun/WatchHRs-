...
from sqlalchemy import create_engine, Column, String, DateTime, Text, ForeignKey, LargeBinary
...
class Photo(Base):
    ...
    data = Column(LargeBinary)  # ← BYTEA: байты файла в БД
...
def main():
    print("=" * 60)
    print("Импорт фото из face_db/ в PostgreSQL (BYTEA)")
    print("=" * 60)
    ...
    try:
        for filename in os.listdir(KNOWN_FACES_DIR):
            ...
            if existing:
                print(f"Уже существует: {employee_name}")
                skipped += 1
                continue
            ...
            employee = Employee(
                id=emp_id,
                name=employee_name.replace("_", " ").title(),
                email=email,
            ...
            db.add(photo)
            imported += 1
            print(f"✅ Добавлен: {employee_name}")

        db.commit()
        print("=" * 60)
        print(f"Итого: {imported} импортировано, {skipped} пропущено")
        print("=" * 60)
